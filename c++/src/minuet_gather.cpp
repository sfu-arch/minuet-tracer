...
MetadataContents read_metadata_cpp(const std::string& filename) {
    ...
    auto read_data = [&](void* buffer, unsigned int size) {
        if (gzread(inFile, buffer, size) != static_cast<int>(size)) {
            gzclose(inFile);
            throw std::runtime_error("Failed to read data from metadata gzip file or unexpected EOF.");
        }
    };

    // Read and verify magic number ("MINU")
    read_data(magic, sizeof(magic));
    if (magic[0] != 'M' || magic[1] != 'I' || magic[2] != 'N' || magic[3] != 'U') {
        gzclose(inFile);
        throw std::runtime_error("Invalid metadata file format: magic number mismatch.");
    }
    ...
    if (contents.version != 1) { // Assuming version 1 for now
        gzclose(inFile);
        throw std::runtime_error("Unsupported metadata file version: " + std::to_string(contents.version));
    }

    // Read num_total_system_offsets, num_total_system_sources, total_slots_in_gemm_buffer
    read_data(&contents.num_total_system_offsets, sizeof(contents.num_total_system_offsets));
    read_data(&contents.num_total_system_sources, sizeof(contents.num_total_system_sources));
    read_data(&contents.total_slots_in_gemm_buffer, sizeof(contents.total_slots_in_gemm_buffer));

    // Read number of active offsets
    read_data(&contents.num_active_offsets_in_map, sizeof(contents.num_active_offsets_in_map));
    ...
    for (uint32_t i = 0; i < contents.num_active_offsets_in_map; ++i) {
        ...
        read_data(&contents.active_offsets_details[i].base_address, sizeof(uint32_t));
        read_data(&contents.active_offsets_details[i].num_matches, sizeof(uint32_t));
    }

    // Calculate mask size and read masks
    if (contents.num_total_system_offsets > 0 && contents.num_total_system_sources > 0) {
        size_t mask_elements = static_cast<size_t>(contents.num_total_system_offsets) * contents.num_total_system_sources;
        if (mask_elements > 0) {
            contents.out_mask.resize(mask_elements);
            read_data(contents.out_mask.data(), static_cast<unsigned int>(mask_elements * sizeof(int32_t)));

            contents.in_mask.resize(mask_elements);
            read_data(contents.in_mask.data(), static_cast<unsigned int>(mask_elements * sizeof(int32_t)));
        }
    } else {
        // Handle cases where masks might be empty if num_total_system_offsets or num_total_system_sources is 0
        // contents.out_mask and contents.in_mask will remain empty (as default initialized or after resize(0))
    }
...
uint32_t write_metadata_cpp(
    ...
    const std::vector<std::tuple<uint32_t, uint32_t, uint32_t>>& active_offset_data,
    ...
    write_data_and_crc(&num_total_system_offsets, sizeof(num_total_system_offsets));
    write_data_and_crc(&num_total_system_sources, sizeof(num_total_system_sources));

    // Total slots allocated for gemm buffer - Little-endian
    write_data_and_crc(&total_slots_in_gemm_buffer, sizeof(total_slots_in_gemm_buffer));

    // Number of active offsets in map - Little-endian
    uint32_t num_active_offsets = static_cast<uint32_t>(active_offset_data.size());
    write_data_and_crc(&num_active_offsets, sizeof(num_active_offsets));

    // For each active offset: Offset key, Base address, Number of matches - Little-endian
    ...
    if (!out_mask.empty()) {
        write_data_and_crc(out_mask.data(), static_cast<unsigned int>(out_mask.size() * sizeof(int32_t)));
    }
    if (!in_mask.empty()) {
        write_data_and_crc(in_mask.data(), static_cast<unsigned int>(in_mask.size() * sizeof(int32_t)));
    }

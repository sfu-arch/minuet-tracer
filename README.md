...
- trace_reader.py (for reading the memory trace file generated by the minuet_mapping.py and c++ version)
...
This project simulates the memory access patterns of a Minuet-like kernel map algorithm. It includes both a c++ version and a python version. The simulation employs a fixed pool of virtual threads to model parallel processing. The primary output is a compressed binary file (`memory_trace.bin.gz` python version or `memory_trace_cpp.bin.gz` c++ version) containing the recorded memory accesses, which can be used for analyzing memory behavior, locality, and potential bottlenecks.
...
Generate the memory_trace_cpp.bin.gz file in the current working directory (which will be the build directory if run from there).
Project Structure
